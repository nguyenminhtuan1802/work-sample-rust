...
impl IsoTpRxMemory {
    ...
    pub fn format_data(&self) -> String {
        // Convert Vec<u8> to slice &[u8]
        let data_slice: &[u8] = &self.data;

        // Format the slice using UpperHex
        format!("{data_slice:02X?}")
    }
}
                    ...
                    match msg {
                        ...
                        IsoTpMessage::ReadBytes(timeout_ms, sender_resp) => {
                            if rx_memory.completed {
                                ...
                                println!("RX done!: {:02X?}", rx_memory.data);
                            ...
                            Some(pci) => {
                                match pci & 0xF0 {
                                    ...
                                    0x10 => {
                                        ...
                                        println!("ISOTP Start frame {data:02X?}");
                                    ...
                                    0x30 => {
                                        ...
                                        println!("ISOTP Flow control {data:02X?}");
                ...
                if let Ok(msg) = can_msg_receiver.try_recv() {
                    match msg {
                        ...
                        CanMessage::WriteFrames(_timeout, f, resp_sender) => {
                            ...
                            // println!("ISOTP Tx: [{:02X?}]", f[0].get_data());
