...
use crate::{
    core::channel::{
        CanChannel, CanFrame, ChannelError, ChannelResult, IsoTPChannel, PacketChannel,
    },
    hardware::{
...
pub struct PcanUsbDevice {
    ...
    has_isotp_channel: Arc<AtomicBool>,
}
...
impl PcanUsbDevice {
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            has_isotp_channel: Arc::new(AtomicBool::new(false)),
        })
    }
}
...
impl Hardware for PcanUsbDevice {
    ...
    fn create_native_iso_tp_channel(&mut self) -> HardwareResult<Box<dyn CanChannel>> {
        self.has_isotp_channel.store(true, Ordering::Relaxed);
        self.create_can_channel()
    }
    ...
    fn is_iso_tp_channel_open(&self) -> bool {
        self.has_isotp_channel.load(Ordering::Relaxed)
    }
...
pub struct PcanUsbScanner {
    driver: HardwareResult<PCanDrvNew>,
                        ...
                        res.push((
                            *dev,
                            HardwareInfo {
                                ...
                                vendor: Some("PEAK-System Technik GmbH".to_string()),
...
impl HardwareScanner<PcanUsbDevice> for PcanUsbScanner {
    ...
    fn open_device_by_index(&self, idx: usize) -> HardwareResult<PcanUsbDevice> {
        match self.cache.get(idx) {
            Some((handle, info)) => self.open_handle(handle, info.clone()),
